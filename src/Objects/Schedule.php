...
class Schedule extends AbstractModel
{
    ...
    protected function setIntervalLength(int $value) {
        if ($this->intervalUnit === self::SCHEDULE_UNIT_DAYS && $value < 7 || $value > 365) {
            throw new InvalidRequestException('Interval Length must be between "7" and "365" for days.');
        }

        if ($this->intervalUnit === self::SCHEDULE_UNIT_MONTHS && $value < 1 || $value > 12) {
            throw new InvalidRequestException('Interval Length must be between "1" and "12" for months.');
        }
